...
async def lifespan(app: FastAPI):
    ...
    try:
        async with engine.begin() as conn:
            ...
            # Migração leve: colunas adicionais de usuários (alinhamento com PDV3)
            try:
                await conn.execute(text("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS nivel INTEGER DEFAULT 1"))
                await conn.execute(text("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS salario DOUBLE PRECISION DEFAULT 0"))
                await conn.execute(text("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS pode_abastecer BOOLEAN DEFAULT FALSE"))
                await conn.execute(text("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS pode_gerenciar_despesas BOOLEAN DEFAULT FALSE"))
                await conn.execute(text("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS pode_fazer_devolucao BOOLEAN DEFAULT FALSE"))
            except Exception as mig_usr_e:
                print(f"Aviso: migração leve de 'usuarios' (nivel/salario/permissoes) falhou: {mig_usr_e}")
